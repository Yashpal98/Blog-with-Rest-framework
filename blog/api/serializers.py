...
class PostSerializer(serializers.ModelSerializer):
    ...
    # For remove the signature from the image url
    """image 	 = serializers.SerializerMethodField('validate_image_url')"""

    class Meta:
        model = Post
        fields = ['pk', 'title', 'content', 'slug', 'username']
	...
	def validate(self, blog_post):
		try:
			...
			if len(body) < MIN_BODY_LENGTH:
				raise serializers.ValidationError({"response": "Enter a content longer than " + str(MIN_BODY_LENGTH) + " characters."})
